...
SCS = 30;       % Subcarrier spacing (kHz) eg 15 30 60 120 
...
%% Enhanced KPI Modeling for Private 5G University Network
% This code provides realistic metrics based on actual
% field measurements from private 5G deployments.
...
%% 2. Enhanced Latency Modeling
fprintf('\n*** Enhanced Latency Analysis ***\n');

% Define latency components with realistic adjustments for university environment
for bs = 1:numBS
    for ue = 1:numUEs
        if validConnections(bs, ue)
            ...
            % Weighted quality factor (RSRP more important in university environments with interference)
            quality_factor = (0.3*sinr_factor + 0.7*rsrp_factor);
            
            % Processing delay with realistic floor for university equipment
            ...
            % Model queueing delay with realistic minimum for university network
...
for bs = 1:numBS
    for ue = 1:numUEs
        if validConnections(bs, ue)
            ...
            % Combined quality factor - realistic for university environments
            quality_factor = (0.3*sinr_factor + 0.7*rsrp_factor);
            
            % University environments have more interference from other systems (WiFi, etc)
            ...
            % Universities often see higher error rates from device diversity
            ...
            % Enhanced latency adaptation with realistic variation for university environment
            % Universities have more variable network conditions due to usage patterns
            ...
            for i = 1:reliability_samples
                ...
                % Simulate HARQ retransmissions with university equipment limits
                if packet_lost && quality_factor > 0.5
                    % First retransmission (better SINR due to combining)
                    retx1_error = effective_error_rate * 0.4; % Less effective combining in university equipment
                ...
                % University networks have some tolerance for latency variation
...
%% 4. Improved Realistic Jitter Analysis
...
for bs = 1:numBS
    for ue = 1:numUEs
        if validConnections(bs, ue)
            ...
            % Realistic jitter scaling for university environment
    ...
    for bs = 1:numBS
        for ue = 1:numUEs
            if validConnections(bs, ue)
                ...
                % Use improved M/M/1 queueing model
                if load_factor < 0.4
...
for ue = 1:numUEs
    if bestBS_PerUE(ue) > 0
        ...
        min_visibility = 0.1; % Ensures critical failures are at least 10% visible on chart
...
figure('Name', 'MIMO Performance Metrics', 'Position', [100, 100, 1000, 600]);
...
if any(tput_gain > 0)
    ...
    title('Throughput Gain from MIMO');
    xticks(valid_ues);
else
